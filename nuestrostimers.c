...
// Funcion para evaluar si se completa un ciclo del timer
char Tm_Hubo_timer()
{
	return (TIFR1 & (1<<OCF1A));
}

// Bajar bandera del registro manualmente
void Tm_Baje_timer()
{
	TIFR1 &=~ (1<<OCF1A);	
}
...
void Tm_Inicie_timeout (Tm_Periodico *ctp,unsigned int tiempo)
{
	...
	ctp->timeout   =  tiempo; //inicio el contador en el numero de conteos 
	                          // deseado
}
